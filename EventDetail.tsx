...
export default function App() {
  ...
  const handleEventLike = (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="space-y-4">
            {bookmarks.length > 0 ? (
              events.filter(e => bookmarks.includes(e.id)).map(event => (
                <div 
                  key={event.id}
                  onClick={() => setSelectedEvent(event)}
        ...
        {selectedEvent ? (
          ...
          >
            <EventDetail 
              ...
              onLike={(id) => handleEventLike({ stopPropagation: () => {} } as React.MouseEvent, id)}
